...
test('Carrinho usa preços efetivos e preserva o checkout atual da main', () => {
  ...
  assert.match(carrinho, /iniciarCheckoutMercadoPago\(\{/);
  assert.match(carrinho, /recuperarTentativaCheckout\(\)/);
  assert.match(carrinho, /checkoutPendente/);
  assert.match(carrinho, /checkoutEmAndamento/);
  assert.match(carrinho, /limparCarrinho\(\);/);
});
