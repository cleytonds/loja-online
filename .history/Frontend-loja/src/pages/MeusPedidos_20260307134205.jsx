...
export default function MeusPedidos({ usuario_id }) {
  ...
  const [modalPedido, setModalPedido] = useState(null); // Pedido selecionado para modal
        ...
        pedidosArray.map(pedido => (
          <div key={pedido.pedido_id} className="pedido-card">
            ...
            <div className="pedido-acoes">
              <button className="btn-detalhes" onClick={() => abrirModal(pedido)}>Detalhes</button>
      ...
      {modalPedido && (
        <div className="modal-overlay" onClick={fecharModal}>
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            ...
            <p>Total: R$ {modalPedido.total.toFixed(2)}</p>
            <p>Status: <span className={`status-${modalPedido.status}`}>{modalPedido.status}</span></p>
