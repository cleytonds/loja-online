...
export default function Carrinho() {
  ...
  async function fetchCarrinho() {
    ...
    try {
      const res = await api.get("/carrinho", { headers: { Authorization: `Bearer ${token}` } });
      setItens(res.data);
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-10">
      ...
      {itens.length === 0 && <p className="text-center text-gray-600 text-xl">Seu carrinho está vazio</p>}

      <div className="max-w-4xl mx-auto space-y-4">
        {itens.map((item) => (
          <div key={item.id} className="bg-white p-6 rounded-xl shadow-lg flex justify-between items-center">
            <div>
        ...
        {itens.length > 0 && (
          <div className="bg-white p-6 rounded-xl shadow-lg flex justify-between items-center font-bold text-xl">
            <span>Total:</span>
            <span>R$ {total}</span>
          </div>
        )}
