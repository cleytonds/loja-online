...
export async function cadastro(req, res) {
  ...
  Usuario.buscarPorEmail(email, async (err, results) => {
    ...
    Usuario.criar(nome, email, hashSenha, tokenConfirmacao, (err2) => {
      ...
      // 6️⃣ Envia email **em segundo plano** para não travar o cadastro
      enviarEmailConfirmacao(email, tokenConfirmacao)
        .then(() => {
          console.log(`✅ Email de confirmação enviado para ${email}`);
        })
        .catch((mailErr) => {
          console.error(`⚠️ Não foi possível enviar email para ${email}:`, mailErr);
          // Aqui você pode registrar em log ou banco para tentar enviar depois
        });
