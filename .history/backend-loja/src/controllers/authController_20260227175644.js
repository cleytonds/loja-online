...
export async function cadastro(req, res) {
  ...
  // 1️⃣ Valida email por regex e domínio permitido
  const regexGeral = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  const regexDominios = /^[^\s@]+@(gmail\.com|yahoo\.com|outlook\.com)$/i;

  if (!regexGeral.test(email) || !regexDominios.test(email)) {
    return res.status(400).json({
      error:
        "Email inválido! Use um email válido do Gmail, Yahoo ou Outlook."
    });
  }
  ...
  Usuario.buscarPorEmail(email, async (err, results) => {
    ...
    Usuario.criar(nome, email, hashSenha, tokenConfirmacao, async (err2) => {
      ...
      // 6️⃣ Envia email em segundo plano (não trava o cadastro)
      ...
      // 7️⃣ Responde imediatamente ao usuário
      res.status(201).json({
        success:
          "Cadastro realizado! Verifique seu email para ativar a conta. " +
          "Use apenas emails válidos do Gmail, Yahoo ou Outlook."
      });
    });
  });
}
