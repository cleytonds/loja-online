...
// Cadastro com validação de email primeiro
export async function cadastro(req, res) {
  const { nome, email, senha } = req.body;

  // 1️⃣ Validação do email primeiro (bloqueia qualquer envio se inválido)
  const regex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  if (!regex.test(email)) {
    return res.status(400).json({
      error: "Email inválido! Por favor, use um email válido (ex: seuemail@gmail.com)."
    });
  }

  // 2️⃣ Verifica se o email já existe
