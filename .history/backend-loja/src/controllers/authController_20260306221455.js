...
export function verificarCodigo(req, res) {
  ...
  // Buscar usuário pelo token e código
  Usuario.buscarPorTokenECodigo(token, codigo, (err, results) => {
    if (err) return res.status(500).json({ error: "Erro no servidor" });
    if (results.length === 0) return res.status(400).json({ error: "Código inválido ou token incorreto" });

    // Ativar usuário
    Usuario.ativarUsuario(results[0].email, (err2) => {
      if (err2) return res.status(500).json({ error: "Erro ao ativar conta" });
      res.json({ mensagem: "Cadastro confirmado com sucesso!" });
    });
  });
}
