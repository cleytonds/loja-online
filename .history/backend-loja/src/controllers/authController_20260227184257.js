...
// Cadastro rápido com validação de email
export async function cadastro(req, res) {
  ...
  if (!regex.test(email)) {
    return res.status(400).json({
      error:
        "Email inválido! Por favor, use um email válido (ex: seuemail@gmail.com)."
    });
  }
  ...
  Usuario.buscarPorEmail(email, async (err, results) => {
    ...
    Usuario.criar(nome, email, hashSenha, tokenConfirmacao, (err2) => {
      ...
      // **Envio do email em background** para não travar o cadastro
