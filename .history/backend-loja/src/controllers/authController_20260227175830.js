...
function dominioValido(email) {
  ...
  return DOMINIOS_VALIDOS.includes(partes[1].toLowerCase());
}
...
export async function cadastro(req, res) {
  ...
  // Regex geral para email
  const regexGeral = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

  if (!regexGeral.test(email) || !dominioValido(email)) {
    return res.status(400).json({
      error:
        "Email inválido! Use um email válido de provedores comuns (Gmail, Yahoo, Outlook, Hotmail, ProtonMail, iCloud, Live)."
    });
  }
  ...
  Usuario.buscarPorEmail(email, async (err, results) => {
    ...
    Usuario.criar(nome, email, hashSenha, tokenConfirmacao, async (err2) => {
      ...
      enviarEmailConfirmacao(email, tokenConfirmacao)
        ...
        .catch((mailErr) =>
          console.error("Erro ao enviar email de confirmação:", mailErr.message)
        );
      ...
      res.status(201).json({
        success:
          ...
          "Use apenas emails válidos de provedores populares."
      });
    });
  });
}
